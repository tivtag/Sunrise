...
using namespace glm::gtx::quaternion;
...
void sr::Camera::SetupProj()
{
	...
	proj = glm::perspective(fovx, AspectRatio(), 0.1f, 1000.0f);
}
...
void sr::Camera::RotateFirstPerson(float headingDegrees, float pitchDegrees)
{
    ...
    {
        const glm::quat rot = angleAxis(headingDegrees, WORLD_YAXIS);
    ...
    {
        const glm::quat rot = angleAxis(pitchDegrees, WORLD_XAXIS);
        orientation = rot * orientation;
    }
}
...
void sr::Camera::RotateOrbit(float headingDegrees, float pitchDegrees, float rollDegrees)
{
    ...
    if(preferTargetYAxisOrbiting)
    {
        if(headingDegrees != 0.0f)
        {
            const glm::quat rot = angleAxis(headingDegrees, targetYAxis);
            orientation = rot * orientation;
        }
